...
from .loss_util import harris_corner_loss
...
class HarrisCorner_loss(_Loss):
    """
    HarrisCorner Loss
    """
    def __init__(self, para):
        super(HarrisCorner_loss, self).__init__()
        self.reduction = 'mean'
    ...
    def forward(self, pred_image, target_image):
        ...
        # 1. 使用Sobel算子计算梯度
        grad_x_pred, grad_y_pred = self.sobel_filter(pred_image)
        grad_x_target, grad_y_target = self.sobel_filter(target_image)

        # 2. 计算Harris响应，处理RGB三通道
        R_pred = []
        R_target = []
        for c in range(pred_image.shape[1]):  # 迭代RGB通道
            R_pred_c = self.harris_response(grad_x_pred[:, c:c + 1, :, :], grad_y_pred[:, c:c + 1, :, :])
            R_target_c = self.harris_response(grad_x_target[:, c:c + 1, :, :], grad_y_target[:, c:c + 1, :, :])
        ...
        # 3. 生成二值角点图
        corner_map_pred = self.threshold_corners(R_pred)
        corner_map_target = self.threshold_corners(R_target)

        # 4. 计算L1距离作为损失
        # loss = torch.abs(corner_map_pred - corner_map_target).mean()
        loss = F.l1_loss(corner_map_pred, corner_map_target, reduction=self.reduction)
    ...
    def harris_response(self, grad_x, grad_y, k=0.06, kernel_size=5, sigma=1.0):
        """
            计算Harris角点响应
        #         参数:
    ...
    def threshold_corners(self, R, threshold=0.01):
        """通过阈值处理生成二值角点图"""
...
class AMPLoss(nn.Module):  # 振幅损失
    ...
    def forward(self, x, y):
        ...
        return l1_loss(x_mag, y_mag, reduction=self.reduction)


class PhaLoss(nn.Module):  # 相位损失
    def __init__(self):
...
class Loss(nn.Module):
    ...
    def _forward_single(self, x, y, prior_z=None, prior=None, valid_flag=False):
        if len(x.shape) == 5:
            b, n, c, h, w = x.shape
        ...
        for i in range(len(self.losses)):
            if valid_flag is True and self.losses_name[i] == 'GAN':
                loss_sub = self.ratios[i] * self.losses[i](x, y, valid_flag)
            elif self.losses_name[i] == 'Diff_loss':
                loss_sub = self.ratios[i] * self.losses[i](prior_z, prior)
            else:
    ...
    def forward(self, x, y, prior_z=None, prior=None, valid_flag=False):
        if isinstance(x, (list, tuple)):
            ...
            return self._forward_single(x, y, prior_z, prior, valid_flag)
