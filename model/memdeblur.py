...
class Model(nn.Module):
    def __init__(self, para):
        ...
        self.downsampling = nn.Sequential(
            ...
            RDB_DS(in_channels=3, growth_rate=16, num_layer=3),
            RDB_DS(in_channels=4 * 3, growth_rate=int(self.n_feats * 3 / 2), num_layer=3)
        )
        ...
        for s, scale in enumerate(self.scales):
            ...
            for i in range(0, t):
                ...
                if scale != 1:  # need to downsample
                    ...
                    key_curr = self.memory.encode_key(downsampled_feature)
...
class Decoder(nn.Module):  # G Decoder
...
class MemoryBank:                                                           
    ...
    def _global_matching(self, mk, qk):                                     
        ...
        # 简化版的L2相似度：-(mk -qk)^2 = 2*mk*qk-mk^2[-qk^2], 最后一项可省略， 因为softmax操作
    ...
    def _readout(self, affinity, mv):                                       
        return torch.bmm(mv, affinity)  # 三维(b, h, w)矩阵乘法                                     
