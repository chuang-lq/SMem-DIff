...
class Model(nn.Module):
    def __init__(self, para):
        ...
        # ----------------- Diffusion branch -----------------
        self.net_le = LE_arch(n_feats=self.mid_channels)
        self.net_ce = CE(n_feats=self.mid_channels)  # n_encoder_res=n_encoder_res
        self.net_d = denoise(n_feats=self.mid_channels, n_denoise_res=2, timesteps=self.diffusion_schedule['timesteps'])

        # apply LDM implementation
        self.diffusion = DDPM_(denoise=self.net_d, condition=self.net_ce, n_feats=self.mid_channels,
    ...
    def forward(self, inputs, gts=None, profile_flag=False):
        ...
        if self.training:
            ...
            priors, _ = self.diffusion(rearrange(inputs, 'b t c h w -> (b t) c h w'), prior_zs)
            prior_zs = rearrange(prior_zs, '(b t) c -> b t c', b=b)
            priors = rearrange(priors, '(b t) c -> b t c', b=b)
        else:
            prior_zs = None
            if gts is not None:
                prior_zs = self.net_le((rearrange(inputs, 'b t c h w -> (b t) c h w'),
            ...
            priors = self.diffusion(rearrange(inputs, 'b t c h w -> (b t) c h w'))
            priors = rearrange(priors, '(b t) c -> b t c', b=b)
        ...
        for i in range(0, t):
            ...
            encoder_outs, k_to_cache, v_to_cache = self.encoder(down_tf2, priors[:, i],
                                                                k_cached=k_cached, v_cached=v_cached)
            for k in range(0, len(k_to_cache)):
                k_cached[k] = k_to_cache[k]
                v_cached[k] = v_to_cache[k]

            decoder_out, k_to_cache, v_to_cache = self.decoder(encoder_outs, priors[:, i],
                                                                k_cached=k_cached, v_cached=v_cached)
            for k in range(0, len(k_to_cache)):
