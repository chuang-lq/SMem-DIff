...
class StateAlignBlock(nn.Module):
    def __init__(self, dim, num_heads, bias, num_frames_tocache, Scale_patchsize=1):
        """
        ...
        self.k2_dwconv = nn.Conv2d(dim * 2, dim * 2, kernel_size=self.window_size + 1, stride=self.window_size,
                                   padding=1,
                                   groups=dim * 2, bias=bias)
        self.q2 = nn.Conv2d(dim, dim * 2, kernel_size=1, bias=bias)
        self.q2_dwconv = nn.Conv2d(dim * 2, dim * 2, kernel_size=self.window_size + 1, stride=self.window_size,
                                   padding=1,
                                   groups=dim * 2, bias=bias)
...
class CausalHistoryModel(nn.Module):
    ...
    def forward(self, x, prior=None, k_cached=None, v_cached=None):
        ...
        X_channel, _, _ = self.ChanAttn(x, prior=prior, k_cached=k, v_cached=v)
...
class FeedForward(nn.Module):
    def __init__(self, dim, FFN_Expand=2, drop_out_rate=0.):
...
# DynamicAttn
class DynamicAttn(nn.Module):
    def __init__(self, dim, dynamic=True, ffn_expansion_factor=2.0, bias=False):
        super(DynamicAttn, self).__init__()

        hidden_features = int(dim * ffn_expansion_factor)
        self.project_in = nn.Conv2d(dim, hidden_features * 2, kernel_size=1, stride=1, padding=0, bias=bias)
        self.project_out = nn.Conv2d(hidden_features, dim, kernel_size=1, stride=1, padding=0, bias=bias)

        if dynamic:
            self.dwconv = DynamicDWConv(hidden_features, 3, 1, hidden_features)
        else:
...
class AttnBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias, LayerNorm_type, num_heads=1, Scale_patchsize=1,
                 attention_type='ChannelAttn', FFW_type="GFFW", num_frames_tocache=1):
        ...
        Supported Attention types:
            ...
            - `ChannelAttn`: Channel Attention for feature refinement.
            - `DynamicAttn`: Uses Dynamic convolution and gating for feature extraction.
        ...
        if attention_type == "ChannelAttn":
            self.attn = ChannelAttention(dim, num_heads, bias)
        elif attention_type == "DynamicAttn":
            self.attn = DynamicAttn(dim, bias=bias)
        elif attention_type == "FHR":  # Caches num_frames_tocache
            self.attn = FrameHistoryRouter(dim, num_heads, bias, num_frames_tocache=2)
...
class LevelBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias, LayerNorm_type, num_blocks,
                 attn_type1="ChannelAttn", attn_type2="CHM", FFW_type="GFFW", num_frames_tocache=1, num_heads=1,
                 Scale_patchsize=1):
...
class LatentCacheBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias, LayerNorm_type, num_blocks,
                 attn_type1="FHR", attn_type2="ChannelAttn", attn_type3="FHR", FFW_type="GFFW", num_frames_tocache=1,
                 num_heads=1):
...
class Encoder(nn.Module):
    def __init__(self, dim, enc_blocks=[2, 4, 2], num_heads=[2, 4, 8],
                 ffn_expansion_factor=2.5, bias=False, LayerNorm_type='WithBias', num_frames_tocache=3):
        super(Encoder, self).__init__()

        self.encoder_level1 = LevelBlock(dim=dim, bias=bias,
                                         ffn_expansion_factor=ffn_expansion_factor,
                                         LayerNorm_type=LayerNorm_type, num_blocks=enc_blocks[0],
                                         attn_type1="DynamicAttn", attn_type2="DynamicAttn",
                                         FFW_type="FFW", num_frames_tocache=num_frames_tocache,
                                         num_heads=num_heads[0])

        self.down1_2 = Downsample(dim)  # From Level 1 to Level 2
        self.encoder_level2 = LevelBlock(dim=int(dim * 2 ** 1), bias=bias,
                                         ffn_expansion_factor=ffn_expansion_factor,
                                         LayerNorm_type=LayerNorm_type, num_blocks=enc_blocks[1],
                                         attn_type1="ChannelAttn", attn_type2="ChannelAttn",
                                         FFW_type="GFFW", num_frames_tocache=num_frames_tocache,
                                         num_heads=num_heads[1])

        self.down2_3 = Downsample(int(dim * 2 ** 1))  # From Level 2 to Level 3
        self.encoder_level3 = LatentCacheBlock(dim=int(dim * 2 ** 2), ffn_expansion_factor=ffn_expansion_factor,
                                               bias=bias, LayerNorm_type=LayerNorm_type, num_blocks=enc_blocks[2],
                                               attn_type1="FHR", attn_type2="ChannelAttn",
                                               attn_type3="FHR", FFW_type="GFFW",
                                               num_frames_tocache=num_frames_tocache,
                                               num_heads=num_heads[2])

    def forward(self, x, prior, k_cached=None, v_cached=None):
        enc1, k1_tocahe, v1_tocahe = self.encoder_level1(x, prior, k_cached[0], v_cached[0])

        x = self.down1_2(enc1)
        enc2, k2_tocahe, v2_tocahe = self.encoder_level2(x, prior, k_cached[1], v_cached[1])

        x = self.down2_3(enc2)
        enc3, k3_tocahe, v3_tocahe, k4_tocahe, v4_tocahe = self.encoder_level3(x, prior,
                                                                               k1_cached=k_cached[2],
                                                                               v1_cached=v_cached[2],
                                                                               k2_cached=k_cached[3],
                                                                               v2_cached=v_cached[3])

        return ([enc1, enc2, enc3],
                [k1_tocahe, k2_tocahe, k3_tocahe, k4_tocahe], [v1_tocahe, v2_tocahe, v3_tocahe, v4_tocahe])


class Decoder(nn.Module):
    def __init__(self, dim, dec_blocks=[2, 4, 2], num_heads=[2, 4, 8], ffn_expansion_factor=2.5,
                 bias=False, LayerNorm_type='WithBias', num_frames_tocache=3):
        ...
        self.decoder_level1 = LevelBlock(dim=int(dim * 1),
                                         ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                         LayerNorm_type=LayerNorm_type, num_blocks=dec_blocks[0],
                                         attn_type1="ChannelAttn", attn_type2="CHM",
                                         FFW_type="GFFW", num_frames_tocache=num_frames_tocache,
                                         num_heads=num_heads[0], Scale_patchsize=4)

        self.up3_2 = Upsample(int(dim * 2 ** 2))  # From Level 3 to Level 2
        self.reduce_chan_level2 = nn.Conv2d(int(dim * 2 ** 2), int(dim * 2 ** 1), kernel_size=1, bias=bias)
        self.decoder_level2 = LevelBlock(dim=int(dim * 2 ** 1),
                                         ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                         LayerNorm_type=LayerNorm_type, num_blocks=dec_blocks[1],
                                         attn_type1="ChannelAttn", attn_type2="CHM",
                                         FFW_type="GFFW", num_frames_tocache=num_frames_tocache,
                                         num_heads=num_heads[1], Scale_patchsize=2)

        self.decoder_level3 = LevelBlock(dim=int(dim * 2 ** 2),
                                         ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                         LayerNorm_type=LayerNorm_type, num_blocks=dec_blocks[2],
                                         attn_type1="ChannelAttn", attn_type2="CHM",
                                         FFW_type="GFFW", num_frames_tocache=num_frames_tocache,
                                         num_heads=num_heads[2], Scale_patchsize=1)

    def forward(self, encs, prior, k_cached=None, v_cached=None):
        enc1, enc2, enc3 = encs

        dec3, k5_tocahe, v5_tocahe = self.decoder_level3(enc3, prior, k_cached[4], v_cached[4])

        dec2 = self.up3_2(dec3)
        dec2 = torch.cat([dec2, enc2], 1)
        dec2 = self.reduce_chan_level2(dec2)
        dec2, k6_tocahe, v6_tocahe = self.decoder_level2(dec2, prior, k_cached[5], v_cached[5])

        dec1 = self.up2_1(dec2)
        dec1 = torch.cat([dec1, enc1], 1)
        dec1 = self.reduce_chan_level1(dec1)
        dec1, k7_tocahe, v7_tocahe = self.decoder_level1(dec1, prior, k_cached[6], v_cached[6])

        return dec1, [k5_tocahe, k6_tocahe, k7_tocahe], [v5_tocahe, v6_tocahe, v7_tocahe]


if __name__ == '__main__':
    x = torch.randn(1, 64, 128, 128)
    net = ConverseBlockNoBN(64)
    o = net(x)
    print(o.shape)
