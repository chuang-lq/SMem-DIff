...
class IDynamicDWConv1(nn.Module):

    def __init__(self, channels, kernel_size=3, group_channels=1, conv_group=1):
        super(IDynamicDWConv1, self).__init__()
        ...
        Block2 = [ResBlock(channels, channels, kernel_size=kernel_size, stride=1, groups=conv_group)
                  for _ in range(3)]
        Block3 = [ResBlock(channels, channels, kernel_size=kernel_size, stride=1, groups=conv_group)
                  for _ in range(3)]
    ...
    def forward(self, x):
        w1 = self.Block1(self.avgpool1(x))
        w2 = self.Block2(self.avgpool2(self.Block1(self.avgpool1(x))))
        w3 = self.Block3(self.avgpool3(self.Block2(self.avgpool2(self.Block1(self.avgpool1(x))))))
        w1 = F.interpolate(w1, scale_factor=2, mode='bilinear')
        w2 = F.interpolate(w2, scale_factor=4, mode='bilinear')
        w3 = F.interpolate(w3, scale_factor=8, mode='bilinear')
        weight = self.tokernel(torch.cat([w1, w2, w3], dim=1))
        b, c, h, w = weight.shape
        weight = weight.view(b, self.groups, self.kernel_size, self.kernel_size, h, w)
        out = _idynamic_cuda(x, weight, stride=1, padding=(self.kernel_size - 1) // 2)
        return out
...
class DTFF(nn.Module):
    ...
    def forward(self, x_feat, feat_prop):
        feat_fusion = torch.cat([x_feat, feat_prop], dim=1)  # b 128 h w
        feat_fusion = self.lrelu(self.conv1(feat_fusion))  # b 128 h w
...
class DTFF1(nn.Module):
    def __init__(self, num_feat=64):
        super(DTFF1, self).__init__()

        self.num_feat = num_feat
        self.conv1x1 = nn.Conv2d(num_feat * 2, num_feat * 2, 1, 1, 0)
        self.conv3x3 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        self.kernel_conv_pixel = IDynamicDWConv1(num_feat, kernel_size=3, group_channels=1, conv_group=1)

    def forward(self, x_feat, feat_prop):
        feat_fusion = torch.cat([x_feat, feat_prop], dim=1)  # b 128 h w
        feat_fusion = self.conv1x1(feat_fusion)  # b 128 h w
        feat_prop1, feat_prop2 = torch.split(feat_fusion, self.num_feat, dim=1)
        feat_prop1 = x_feat * torch.sigmoid(feat_prop1 * feat_prop2)
        feat_prop2 = feat_prop * torch.sigmoid(1 - feat_prop1 * feat_prop2)
        feat_prop = self.conv3x3(feat_prop1 + feat_prop2)
        # dynamic conv
        feat_prop = self.kernel_conv_pixel(feat_prop)
...
if __name__ == '__main__':
    # net = IDynamicDWConv(channels=64, kernel_size=3, group_channels=1, down=4, conv_group=1).cuda()
    net1 = DTFF(num_feat=64).cuda()
    net2 = DTFF1(num_feat=64).cuda()
    x = torch.randn(2, 64, 256, 256).cuda()
    y = torch.randn(2, 64, 256, 256).cuda()
    h1 = net1(x, y)
    h2 = net2(x, y)
    print(h1.shape)
    print(h2.shape)
